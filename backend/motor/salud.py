...
Un score sin metodología visible no se puede defender: es un número que hay
...
bibliográfica, y todo eso se sirve al frontend para pintarlo al lado de la
...
Como se arma la nota de cada indicador:
  ...
  - NIVEL: que tan lejos esta el valor del último periodo de un rango
    considerado sano por la bibliografía. Se interpola linealmente entre los
    puntos de una escala, para que no haya saltos bruscos: una razón corriente
    de 1,49 no puede sacar 40 y una de 1,51 sacar 85.
  - TENDENCIA: si mejoro o empeoro entre el primer periodo y el último, medido
    en variacion relativa y orientado según lo que sea deseable en cada caso
    (en el ciclo de caja bajar es mejorar).

La nota de cada dimensión es el promedio ponderado de sus indicadores, y el
...
entre los que si tienen dato. Castigar por un dato ausente sería confundir
"no informado" con "malo". Lo excluido se reporta.

Determinista, sin intervencion de ningun modelo de lenguaje.
Criterios y fuentes en FÓRMULAS.md.
"""
...
class Criterio:
    """Un indicador que participa en el puntaje, con su regla de calificación."""
...
CRITERIOS: tuple[Criterio, ...] = (
    ...
    Criterio(
        ...
        justificacion="Mide lo mismo sin contar el inventario, que es el activo corriente "
                      "más lento de volverse caja. Se considera sana a partir de 1 vez: la "
                      "empresa paga sin tener que vender mercancía a las carreras.",
        fuente=BAENA,
    ),
    Criterio(
        codigo="ciclo_conversion_efectivo", dimension="Actividad", peso=45,
        escala=((0, 100), (30, 88), (60, 70), (90, 48), (120, 28), (180, 5)),
        justificacion="Días que la plata pasa fuera de la caja. Entre más largo, más capital "
                      "de trabajo hay que financiar, y esa financiación cuesta. Un ciclo "
                      "negativo o cercano a cero es la mejor posición posible: el proveedor "
    ...
    Criterio(
        ...
        justificacion="Centavos de capital de trabajo que exige cada peso de venta. Garcia "
                      "lo usa para saber si crecer conviene: con una PKT alta, vender más "
                      "consume más caja de la que produce.",
        fuente=GARCIA, mejora="baja",
    ),
    Criterio(
    ...
    Criterio(
        ...
        justificacion="Porcentaje del activo financiado por terceros. Se considera manejable "
                      "hasta 50-60%. Cero deuda tampoco saca la nota máxima: la deuda barata "
    ...
    Criterio(
        ...
        justificacion="Cuántas veces la utilidad operacional alcanza a pagar los intereses. "
                      "Por debajo de 2 veces es zona de riesgo: cualquier tropiezo de la "
                      "operación deja a la empresa sin con que pagarle al banco.",
        fuente=GARCIA,
    ),
    Criterio(
    ...
    Criterio(
        ...
        justificacion="Lo que finalmente queda para los socios después de intereses e "
                      "impuestos. La distancia con el margen operacional mide cuánto se "
                      "lleva la financiación.",
        fuente=BAENA,
    ),
    Criterio(
    ...
    Criterio(
        codigo="spread_roic", dimension="Generación de valor", peso=100,
...
PESOS_DIMENSION = {
    ...
    "Endeudamiento": 25.0,
    "Rentabilidad": 25.0,
    "Generación de valor": 15.0,
}

NOMBRES = {
    "razon_corriente": "Razón corriente",
    "prueba_acida": "Prueba ácida",
    "ciclo_conversion_efectivo": "Ciclo de conversión de efectivo",
...
def _puntaje_tendencia(inicial, final, mejora: str):
    """Nota 0-100 por como evoluciono el indicador entre el primer periodo y el último.
...
def banda(puntaje: float) -> dict:
    ...
    for piso, nombre, color in BANDAS:
        ...
        anterior = piso
    return {"nombre": "Crítica", "color": "rojo", "rango": "0 a 40"}
...
def puntaje_salud(ef: EstadosFinancieros) -> dict:
    """Puntaje 0-100 con toda su metodología expuesta.
    ...
    for nombre, peso in PESOS_DIMENSION.items():
        ...
        if not vivas or suma_pesos == 0:
            dimensiones.append({
                ...
                "evaluable": False,
                "motivo": "Ningun indicador de esta dimensión tiene datos suficientes.",
                "criterios": propias,
            })
            continue
    ...
    if not vivas or peso_vivo == 0:
        return {
            "disponible": False,
            "motivo": "No hay datos suficientes para calificar ninguna dimensión.",
